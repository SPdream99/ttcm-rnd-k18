...
export default function TeacherUploadCenterPage() {
  ...
  const handleImageFileRead = (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      if (!file.type.startsWith("image/")) {
        toast.error("Vui lòng chọn file hình ảnh (PNG, JPG, WebP, ...)", "Lỗi File");
        reject(new Error("Invalid file type"));
        return;
      }
      if (file.size > 5 * 1024 * 1024) {
        toast.warning("Dung lượng ảnh vượt quá 5MB. Nên chọn ảnh nhỏ hơn để tối ưu.", "Cảnh Báo Dung Lượng");
      }
  ...
  const handlePairQuestionImageFile = async (pairIdx: number, file: File) => {
    try {
      const dataUrl = await handleImageFileRead(file);
      setPairs((prev) => {
        const copy = [...prev];
        copy[pairIdx] = {
          ...copy[pairIdx],
          image_url: dataUrl,
          imageUrl: dataUrl,
          question_image_url: dataUrl,
          questionImageUrl: dataUrl,
        };
        return copy;
      });
      toast.success("Đã tải ảnh câu hỏi thành công!", "Hình Ảnh Câu Hỏi");
    } catch {}
  };

  const handlePairRightAnswerImageFile = async (pairIdx: number, file: File) => {
    try {
      const dataUrl = await handleImageFileRead(file);
      setPairs((prev) => {
        const copy = [...prev];
        copy[pairIdx] = {
          ...copy[pairIdx],
          right_answer_image_url: dataUrl,
          rightAnswerImageUrl: dataUrl,
        };
        return copy;
      });
      toast.success("Đã tải ảnh đáp án đúng thành công!", "Hình Ảnh Đáp Án");
    } catch {}
  };

  const handlePairDistractionImageFile = async (pairIdx: number, distIdx: number, file: File) => {
    try {
      const dataUrl = await handleImageFileRead(file);
      setPairs((prev) => {
        const copy = [...prev];
        const distImgs = [...(copy[pairIdx].wrong_answers_image_urls || copy[pairIdx].wrongAnswersImageUrls || copy[pairIdx].distraction_image_urls || [])];
        while (distImgs.length <= distIdx) {
          distImgs.push("");
        }
        distImgs[distIdx] = dataUrl;
        copy[pairIdx] = {
          ...copy[pairIdx],
          wrong_answers_image_urls: distImgs,
          wrongAnswersImageUrls: distImgs,
          distraction_image_urls: distImgs,
          distractionImageUrls: distImgs,
        };
        return copy;
      });
      toast.success(`Đã tải ảnh cho đáp án sai #${distIdx + 1}!`, "Hình Ảnh Đáp Án Sai");
    } catch {}
  };

  const handleUpdateDistractionImage = (pairIdx: number, distIdx: number, val: string) => {
    setPairs((prev) => {
      const copy = [...prev];
      const distImgs = [...(copy[pairIdx].wrong_answers_image_urls || copy[pairIdx].wrongAnswersImageUrls || copy[pairIdx].distraction_image_urls || [])];
      while (distImgs.length <= distIdx) {
  ...
  const handleAddDistraction = (pairIdx: number) => {
    setPairs((prev) => {
      ...
      const distImgs = [...(copy[pairIdx].wrong_answers_image_urls || copy[pairIdx].distraction_image_urls || []), ""];
      copy[pairIdx].wrong_answers_image_urls = distImgs;
      copy[pairIdx].wrongAnswersImageUrls = distImgs;
      copy[pairIdx].distraction_image_urls = distImgs;
      copy[pairIdx].distractionImageUrls = distImgs;
      return copy;
    });
  };
  ...
  const handleRemoveDistraction = (pairIdx: number, distIdx: number) => {
    setPairs((prev) => {
      const copy = [...prev];
      copy[pairIdx].distractions = (copy[pairIdx].distractions || []).filter((_: string, i: number) => i !== distIdx);
      const distImgs = (copy[pairIdx].wrong_answers_image_urls || copy[pairIdx].distraction_image_urls || []).filter((_: string, i: number) => i !== distIdx);
      copy[pairIdx].wrong_answers_image_urls = distImgs;
      copy[pairIdx].wrongAnswersImageUrls = distImgs;
      copy[pairIdx].distraction_image_urls = distImgs;
      copy[pairIdx].distractionImageUrls = distImgs;
            ...
            <div className="space-y-4">
              {pairs.map((pair, idx) => (
                <div key={pair.id || idx} className="p-5 rounded-xl bg-zinc-50 border border-zinc-200 space-y-3">
                    ...
                    <div className="p-3 rounded-xl bg-white border border-zinc-200 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[11px] font-bold text-zinc-600 flex items-center gap-1.5">
                          <ImageIcon className="w-3.5 h-3.5 text-red-600" /> Hình ảnh cho Câu Hỏi (Tùy chọn)
                        </span>
                        {(pair.image_url || pair.question_image_url) && (
                          <button
                            type="button"
                            onClick={() => {
                              handleUpdatePair(idx, "image_url", "");
                              handleUpdatePair(idx, "question_image_url", "");
                            }}
                      ...
                      <div className="flex items-center gap-2">
                        <label className="px-3 py-1.5 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-zinc-700 text-xs font-bold cursor-pointer flex items-center gap-1.5 transition-colors border border-zinc-300 shrink-0">
                          <Upload className="w-3.5 h-3.5 text-zinc-600" /> Tải ảnh lên
                          <input
                            type="file"
                            accept="image/*"
                            className="hidden"
                            onChange={(e) => {
                              if (e.target.files?.[0]) {
                                handlePairQuestionImageFile(idx, e.target.files[0]);
                              }
                            }}
                          />
                        </label>
                        <input
                          type="text"
                          value={pair.image_url || pair.question_image_url || ""}
                          onChange={(e) => {
                            handleUpdatePair(idx, "image_url", e.target.value);
                            handleUpdatePair(idx, "question_image_url", e.target.value);
                          }}
                          placeholder="Hoặc dán URL hình ảnh (https://...)"
                          className="w-full bg-zinc-50 border border-zinc-200 focus:border-red-600 rounded-lg px-3 py-1.5 text-xs text-zinc-900 focus:outline-none"
                        />
                      </div>

                      {(pair.image_url || pair.question_image_url) && (
                        <div className="relative w-24 h-24 rounded-lg overflow-hidden border border-zinc-300 bg-zinc-100 mt-1">
                          <img
                            src={pair.image_url || pair.question_image_url}
                  ...
                  {/* Right Answer Text & Right Answer Image */}
                  <div className="p-3.5 rounded-xl bg-emerald-50/50 border border-emerald-200 space-y-2">
                    ...
                    <input
                      type="text"
                      value={pair.description || pair.rightAnswer || ""}
                      onChange={(e) => {
                        handleUpdatePair(idx, "description", e.target.value);
                        handleUpdatePair(idx, "rightAnswer", e.target.value);
                      }}
                    ...
                    {/* Right Answer Image Input & Preview */}
                    <div className="pt-1">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[11px] font-bold text-emerald-700 flex items-center gap-1.5">
                          <ImageIcon className="w-3.5 h-3.5 text-emerald-600" /> Hình ảnh cho Đáp Án Đúng (Tùy chọn)
                        </span>
                        {(pair.right_answer_image_url || pair.rightAnswerImageUrl) && (
                          <button
                            type="button"
                            onClick={() => {
                              handleUpdatePair(idx, "right_answer_image_url", "");
                              handleUpdatePair(idx, "rightAnswerImageUrl", "");
                            }}
                            className="text-[10px] text-red-600 hover:underline font-bold flex items-center gap-1"
                          >
                            <X className="w-3 h-3" /> Xóa ảnh
                      ...
                      <div className="flex items-center gap-2">
                        <label className="px-3 py-1.5 rounded-lg bg-emerald-100 hover:bg-emerald-200 text-emerald-800 text-xs font-bold cursor-pointer flex items-center gap-1.5 transition-colors border border-emerald-300 shrink-0">
                          <Upload className="w-3.5 h-3.5 text-emerald-700" /> Tải ảnh đáp án
                          <input
                            type="file"
                            accept="image/*"
                            className="hidden"
                            onChange={(e) => {
                              if (e.target.files?.[0]) {
                                handlePairRightAnswerImageFile(idx, e.target.files[0]);
                              }
                            }}
                          />
                        </label>
                        <input
                          type="text"
                          value={pair.right_answer_image_url || pair.rightAnswerImageUrl || ""}
                          onChange={(e) => {
                            handleUpdatePair(idx, "right_answer_image_url", e.target.value);
                            handleUpdatePair(idx, "rightAnswerImageUrl", e.target.value);
                          }}
                          placeholder="Dán URL hình ảnh cho đáp án đúng..."
                          className="w-full bg-white border border-emerald-200 focus:border-emerald-600 rounded-lg px-3 py-1.5 text-xs text-zinc-900 focus:outline-none"
                        />
                      </div>

                      {(pair.right_answer_image_url || pair.rightAnswerImageUrl) && (
                        <div className="relative w-20 h-20 rounded-lg overflow-hidden border border-emerald-300 bg-white mt-1.5">
                          <img
                            src={pair.right_answer_image_url || pair.rightAnswerImageUrl}
                            alt="Right answer preview"
                            className="w-full h-full object-cover"
                          />
                        </div>
                      )}
                  ...
                  {/* Wrong Answers / Distractions with Image Upload */}
                  <div className="space-y-3 pt-3 border-t border-zinc-200">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-zinc-700 flex items-center gap-1.5">
                        Các Phương Án Sai & Hình Ảnh Đi Kèm:
                      </label>
                      <button
                        type="button"
                    ...
                    <div className="space-y-3">
                      {(pair.distractions || []).map((dist, dIdx) => {
                        const distImgs = pair.wrong_answers_image_urls || pair.wrongAnswersImageUrls || pair.distraction_image_urls || [];
                        const currentDistImg = distImgs[dIdx] || "";

                        return (
                          <div key={dIdx} className="p-3 rounded-xl bg-white border border-zinc-200 space-y-2">
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                placeholder={`Nội dung phương án sai #${dIdx + 1}`}
                                className="w-full bg-zinc-50 border border-zinc-200 focus:border-red-600 rounded-lg px-3 py-1.5 text-xs text-zinc-900 focus:outline-none"
                              />
                            ...
                            <div className="flex items-center gap-2 pt-1 border-t border-zinc-100">
                              <label className="px-2.5 py-1 rounded bg-zinc-100 hover:bg-zinc-200 text-zinc-700 text-[11px] font-bold cursor-pointer flex items-center gap-1 border border-zinc-300 shrink-0">
                                <Upload className="w-3 h-3 text-zinc-600" /> Tải ảnh
                                <input
                                  type="file"
                                  accept="image/*"
                                  className="hidden"
                                  onChange={(e) => {
                                    if (e.target.files?.[0]) {
                                      handlePairDistractionImageFile(idx, dIdx, e.target.files[0]);
                                    }
                                  }}
                                />
                              </label>
                              <input
                                type="text"
                                value={currentDistImg}
                                onChange={(e) => handleUpdateDistractionImage(idx, dIdx, e.target.value)}
                                placeholder="Dán URL hình ảnh phương án sai này..."
                                className="w-full bg-zinc-50 border border-zinc-200 focus:border-red-600 rounded-lg px-2.5 py-1 text-[11px] text-zinc-900 focus:outline-none"
                              />
                              {currentDistImg && (
                                <button
                                  type="button"
                                  onClick={() => handleUpdateDistractionImage(idx, dIdx, "")}
                            ...
                            {currentDistImg && (
                              <div className="relative w-16 h-16 rounded-md overflow-hidden border border-zinc-200 bg-zinc-100">
                                <img
                                  src={currentDistImg}
                                  alt={`Distraction #${dIdx + 1}`}
                                  className="w-full h-full object-cover"
                                />
                              </div>
                            )}
