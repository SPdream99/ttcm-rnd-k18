...
export default function StudentProfilePage() {
  ...
  const [displayName, setDisplayName] = useState('Nguyễn Văn An');
    ...
    <div className="min-h-screen bg-[#0a0e1a] text-white p-6 md:p-10 font-sans">
      <header className="max-w-4xl mx-auto flex justify-between items-center pb-6 border-b border-sky-500/20 mb-8">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-sm text-white font-bold">STUDENT PROFILE</span>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto space-y-6">
        <div className="p-8 rounded-3xl bg-[#0f1524]/80 border border-sky-500/30 backdrop-blur-xl shadow-2xl space-y-6">
          ...
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-sky-500/20 border border-sky-400 flex items-center justify-center text-sky-300 text-3xl">
              🎓
            </div>
            <div>
              <h1 className="text-2xl font-bold text-white">{displayName}</h1>
              <p className="font-mono text-xs text-sky-300">Lớp: Chuyên Tin 12A1 • Mã HS: HS-2026-001</p>
            </div>
          </div>
          ...
          <div className="flex gap-2 p-1 rounded-xl bg-slate-950 border border-slate-800 font-mono text-xs">
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg transition-all ${
                tab === 'info' ? 'bg-sky-500 text-black font-bold shadow-md' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg transition-all ${
                tab === 'password' ? 'bg-sky-500 text-black font-bold shadow-md' : 'text-slate-400 hover:text-white'
              }`}
            >
          ...
          {msg && (
            <div className="p-4 rounded-xl bg-sky-500/15 border border-sky-500/40 text-sky-300 text-xs font-mono">
              {msg}
            </div>
          )}
            ...
            <form onSubmit={handleSaveInfo} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-sky-400"
                />
              </div>
              <div>
                <label className="block text-xs font-mono text-slate-300 mb-1">Email Được Nhà Trường Cấp</label>
                <input
                  type="email"
                  value={email}
              ...
              <button
                type="submit"
                className="px-6 py-3 rounded-xl bg-sky-500 hover:bg-sky-400 text-black font-bold font-mono text-xs transition-all"
              >
