...
import { Rocket, GraduationCap, UserCheck, CheckCircle2, ShieldAlert, Sparkles, UserPlus } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const [step, setStep] = useState<1 | 2>(1); // Step 1: Form info, Step 2: 2FA Simulation
  const [mockSentCode] = useState("123456");
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (verificationCode !== mockSentCode) {
      setMessage("Mã xác thực không chính xác. Vui lòng nhập: 123456");
      return;
    }
    ...
    if (!res.success || !res.user) {
      setMessage(res.error || "Đăng ký không thành công.");
      return;
    }
    ...
    if (role === "teacher") {
      setMessage("Đăng ký thành công! Đang chuyển hướng sang trang chờ Admin phê duyệt...");
      setTimeout(() => {
        window.location.href = "/pending";
      }, 1200);
    } else {
      setMessage("Đăng ký thành công! Đang chuyển hướng vào Trung tâm Học sinh...");
      setTimeout(() => {
      ...
      <main className="w-full max-w-lg relative z-10 my-auto">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-xl md:text-2xl font-bold text-white mb-1">
            Khởi Tạo Hồ Sơ Chỉ Huy
          </h2>
          <p className="text-xs md:text-sm text-[#8e9bb4]">
            Chọn vai trò và gia nhập hệ sinh thái giáo dục thế hệ mới.
          </p>
        </div>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div>
                  <div className="text-xs font-bold font-mono text-white">Tôi là Học sinh</div>
                  <div className="text-[10px] text-cyan-300/80">Tham gia học & chơi</div>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-[10px] text-emerald-300/80">Tạo đề & lộ trình</div>
                </div>
              </button>
            </div>
            ...
            {role === "teacher" && (
              <div className="mt-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-start gap-2.5 animate-fade-in">
                ...
                <div className="text-xs text-amber-200">
                  <span className="font-bold">Lưu ý:</span> Tài khoản Giáo viên cần được <span className="underline">Admin phê duyệt</span> sau khi đăng ký trước khi có thể truy cập hệ thống.
                </div>
              </div>
            )}
          </div>
          ...
          {step === 1 ? (
            <form onSubmit={handleNextStep} className="space-y-4">
              ...
              {role === "teacher" && (
                <div>
                  <label className="block text-xs font-mono text-slate-300 mb-1">
                    Mã Trường / Bộ Môn (Không bắt buộc)
                  </label>
                  <input
                    type="text"
                    value={schoolCode}
                    onChange={(e) => setSchoolCode(e.target.value)}
                    placeholder="VD: HIGH_SCHOOL_K18"
                    className="w-full bg-[#151b2c] border border-cyan-500/20 focus:border-cyan-400 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-1 focus:ring-cyan-400 transition-all placeholder-slate-500"
                  />
                </div>
              )}
              ...
              <button
                type="submit"
                className="w-full mt-2 py-3 rounded-xl bg-gradient-to-r from-blue-600 via-cyan-500 to-teal-400 hover:from-blue-500 hover:to-cyan-300 text-white font-bold font-mono text-sm shadow-[0_0_20px_rgba(6,182,212,0.35)] transition-all hover:scale-[1.01] cursor-pointer flex items-center justify-center gap-2"
              >
                Tiếp Tục Xác Thực →
              </button>
            </form>
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-4">
              <div className="p-4 rounded-xl bg-[#151b2c] border border-cyan-500/20 text-center">
                <div className="text-xs text-slate-400 mb-1">Nhập mã xác thực 6 số gửi đến:</div>
                <div className="font-bold text-sm text-cyan-300 font-mono">{email}</div>
                <div className="text-[11px] text-slate-500 mt-2">Mã thử nghiệm mô phỏng: <span className="font-mono font-bold text-white">123456</span></div>
              </div>
              ...
              <div>
                <input
                  ...
                  placeholder="123456"
                  className="w-full text-center tracking-[0.5em] font-mono text-xl bg-[#151b2c] border border-cyan-500/40 focus:border-cyan-400 rounded-xl py-3 text-white focus:outline-none focus:ring-1 focus:ring-cyan-400 transition-all"
                  required
                />
              </div>
