...
export default function TeacherProfilePage() {
  const { profile, updateProfile, loading } = useAuthAdapter("usr_teacher");
  const [fullName, setFullName] = useState(profile?.fullName || "ThS. Phạm Hoàng Nam");
  const [email, setEmail] = useState(profile?.email || "nam.ph@eve.edu.vn");
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0a0e1a] text-[#e1e2ec] flex items-center justify-center font-sans">
        <p className="text-emerald-400 font-medium">Đang tải hồ sơ giảng viên...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0e1a] text-[#e1e2ec] p-4 md:p-8 font-sans space-y-8 max-w-4xl mx-auto">
      ...
      <div className="p-6 rounded-2xl bg-[#0f1524]/60 backdrop-blur-md border border-[#7bd1fa]/15 flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="space-y-1 text-center sm:text-left">
          <h2 className="text-xl font-bold text-white flex items-center justify-center sm:justify-start gap-2">
            {profile?.fullName || "ThS. Phạm Hoàng Nam"} <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-xs border border-emerald-500/30">Giảng Viên Tiêu Biểu</span>
          </h2>
      ...
      <div className="p-6 rounded-2xl bg-[#0f1524]/60 backdrop-blur-md border border-[#7bd1fa]/15 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-[#151b2c] border border-[#7bd1fa]/20 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-400"
            />
          </div>
          <div>
        ...
        <button
          onClick={handleSave}
          className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-medium text-sm shadow-[0_0_15px_rgba(16,185,129,0.3)] transition-all flex items-center gap-2"
        >
