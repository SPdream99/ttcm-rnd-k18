...
import { FileText, Clock, Upload, Check } from "lucide-react";
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0a0e1a] text-[#e1e2ec] flex items-center justify-center font-sans">
        <p className="text-cyan-400 font-medium">Đang tải bài tập...</p>
      </div>
    );
  }
      ...
      <div className="space-y-4">
        {assignments
          ...
          .map((item) => (
            <div
              key={item.id}
              className="p-5 rounded-2xl bg-[#0f1524]/60 backdrop-blur-md border border-[#7bd1fa]/15 hover:border-cyan-500/30 transition-all space-y-4"
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    {item.score && <span className="text-xs text-[#8e9bb4]">• Thang điểm: {item.score}</span>}
                  </div>
