...
function initSDK() {
  try {
    ...
    GameState.sdk.initSession().then((data) => {
...
function loadNextQuestion() {
  ...
  const bank = GameState.questionBank.length > 0 ? GameState.questionBank : DEFAULT_FALLBACK_PAIRS;
  const randomIndex = Math.floor(Math.random() * bank.length);
  const pair = bank[randomIndex];
...
function handleAnswerSelect(selected, correct, btnElem) {
  ...
  if (isRight) {
    ...
    if (GameState.selectedWeapon === "sword") {
      ...
      // Magic Staff perk: +1.5s time extension
      GameState.marathonTimeRemaining += 1.5;
      showFloatingDamage(`+1.5s 🪄`, "#38bdf8");
    }
    ...
    GameState.combo = 0;
    playSound("wrong");
    showFloatingDamage("MISS! 0 DMG", "#f43f5e");
  }
...
function startDodgePhase() {
  ...
  GameState.dodgeTimeRemaining = GameState.dodgeTimeMax;

  UI.dodge.targetIcon.innerText = chosen.icon;
  UI.dodge.targetName.innerText = chosen.name;
  UI.dodge.barInner.style.width = "100%";
